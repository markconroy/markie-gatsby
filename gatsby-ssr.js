  ...
  window.onload = () => {
    ...
    !(function () {
      ...
      function onElement(selector, callback) {
        ...
        const els = qsa(root, listener.selector)
        let i = 0
        let el
        while ((el = els[i++])) {
      ...
      function checkListener(listener, target) {
        ...
        if (loaded) {
          ...
          Array.prototype.push.apply(
            els,
            qsa(target || root, listener.selector)
          )
        }
        while ((el = els[i++])) {
      ...
      onElement('[ie-style]', function (el) {
        const newCss = rewriteCss(`{${el.getAttribute('ie-style')}`).substr(1)
        el.style.cssText += `;${newCss}`
        const found = parseRewrittenStyle(el.style)
      ...
      function parseRewrittenStyle(style) {
        ...
        const { cssText } = style
        const matchesGetters = cssText.match(regRuleIEGetters)
        let j
        let match
        if (matchesGetters) {
        ...
        const matchesSetters = cssText.match(regRuleIESetters)
        if (matchesSetters) {
      ...
      function activateStyleElement(style, css) {
        ...
        let i = 0
        let rule // cssRules = CSSRuleList, rules = MSCSSRuleList
        while ((rule = rules[i++])) {
      ...
      function redrawStyleSheets() {
        for (const prop in styles_of_getter_properties) {
      ...
      function selectorAddPseudoListeners(selector) {
        ...
        for (const pseudo in pseudos) {
          const parts = selector.split(`:${pseudo}`)
          if (parts.length > 1) {
            const ending = parts[1].match(/^[^\s]*/) // ending elementpart of selector (used for not(:active))
      ...
      function _drawElement(el) {
        ...
        for (const prop in el.ieCPSelectors) {
          const important = style[`-ieVar-❗${prop}`]
      ...
      function drawTree(target) {
        ...
        const els = target.querySelectorAll('[iecp-needed]')
      ...
      function styleComputeValueWidthVars(style, valueWithVars, details) {
        return findVars(valueWithVars, function (
          variable,
          fallback,
          insideCalc
        ) {
          ...
          if (
            details &&
            style.lastPropertyServedBy !== document.documentElement
          )
      ...
      addEventListener('hashchange', function (e) {
        const newEl = document.getElementById(location.hash.substr(1))
        if (newEl) {
          const oldEl = document.getElementById(oldHash.substr(1))
          drawTree(newEl)
          drawTree(oldEl)
        } else {
          drawTree(document)
      ...
      const styleGetter = descriptor.get
      ...
      const originalGetComputed = getComputedStyle
      window.getComputedStyle = function (el) {
        const style = originalGetComputed.apply(this, arguments)
      ...
      StyleProto.setProperty = function (property, value, prio) {
        ...
        property = `-ie-${prio === 'important' ? '❗' : ''}${property.substr(
          2
        )}`
